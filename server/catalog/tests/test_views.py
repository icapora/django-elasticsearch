...
class ViewTests(APITestCase):
    ...
    def test_empty_query_returns_everything(self):
        response = self.client.get(f'{self.base_url}/pg-wines/')
        wines = Wine.objects.all()
        self.assertEquals(response.data.get('results'), WineSerializer(wines, many=True).data)

    def test_query_matches_variety(self):
        response = self.client.get(f'{self.base_url}/pg-wines/?query=Cabernet')
        self.assertEquals(1, len(response.data.get('results')))
        self.assertEquals("58ba903f-85ff-45c2-9bac-6d0732544841", response.data['results'][0]['id'])

    def test_query_matches_winery(self):
        response = self.client.get(f'{self.base_url}/pg-wines/?query=Barnard')
        self.assertEquals(1, len(response.data.get('results')))
        self.assertEquals("21e40285-cec8-417c-9a26-4f6748b7fa3a", response.data['results'][0]['id'])

    def test_query_matches_description(self):
        response = self.client.get(f'{self.base_url}/pg-wines/?query=wine')
        ...
        ], [item['id'] for item in response.data.get('results')])

    def test_can_filter_on_country(self):
        response = self.client.get(f'{self.base_url}/pg-wines/?country=France')
        self.assertEquals(2, len(response.data.get('results')))
        self.assertEquals("0082f217-3300-405b-abc6-3adcbecffd67", response.data['results'][0]['id'])

    def test_can_filter_on_points(self):
        response = self.client.get(f'{self.base_url}/pg-wines/?points=87')
        self.assertEquals(1, len(response.data.get('results')))
        self.assertEquals("21e40285-cec8-417c-9a26-4f6748b7fa3a", response.data['results'][0]['id'])

    def test_country_must_be_exact_match(self):
        response = self.client.get(f'{self.base_url}/pg-wines/?country=Frances')
    ...
    def test_search_results_returned_in_correct_order(self):
        response = self.client.get(f'{self.base_url}/pg-wines/?query=Chardonnay')
        self.assertEquals(2, len(response.data.get('results')))
    ...
    def test_description_highlights_matched_words(self):
        response = self.client.get(f'{self.base_url}/pg-wines/?query=wine')
        self.assertEquals(
            'A creamy <mark>wine</mark> with full Chardonnay flavors.', response.data['results'][0]['description']
        )
    ...
    def test_suggests_words_for_spelling_mistakes(self):
        ...
        response = self.client.get(f'{self.base_url}/pg-wine-search-words/?query=greegio')
...
class ESViewTests(APITestCase):
    ...
    def test_query_matches_variety(self):
        response = self.client.get(f'{self.base_url}/wines/?query=Cabernet')
        self.assertEquals(1, len(response.data))
        self.assertEquals("58ba903f-85ff-45c2-9bac-6d0732544841", response.data[0]['id'])

    def test_search_results_returned_in_correct_order(self):
        response = self.client.get(f'{self.base_url}/wines/?query=Chardonnay')
    ...
    def test_description_highlights_matched_words(self):
        response = self.client.get(f'{self.base_url}/wines/?query=wine')
        self.assertEquals('A delicious bottle of <mark>wine</mark>.', response.data[0]['description'])

    def test_suggests_words_for_spelling_mistakes(self):
        response = self.client.get(f'{self.base_url}/es-wine-search-words/?query=greegio')
        breakpoint()
